...
const HighLighter = () => {
    ...
    const setCheckedIsToggled = (event) => {
    ...
    const checkDrug = drugType.map((d) =>
        <li>
            <label>
                <input type="checkbox"
                    checked={checked}
                    onChange={setCheckedIsToggled}
                 />{d}</label></li>);
    ...
    const HighLightTag = ({ children, highlightIndex }) => {
        //console.log(props);
        console.log("index and child", children, highlightIndex);

        const HighLightTagStyle = { backgroundColor: 'pink', color: 'green', zIndex: {highlightIndex} }
        // return <strong className="highlighted-text" z-index={highlightIndex}>{children}</strong>
